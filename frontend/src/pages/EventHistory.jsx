      ...
      try {
        ...
        if (evt.long_summary)
          sections.push({
            title: "Long Summary",
            type: "text",
            content: evt.long_summary,
          });
        if (evt.highlights)
          sections.push({
            title: "Highlights",
            type: "text",
            content: evt.highlights,
          });
        if (evt.lessons)
          sections.push({
        ...
        const historyRes = await fetch(
          `http://localhost:5000/api/history/${eventId}`
        );
        ...
        if (historyPayload.exists) {
          ...
          if (h.long_summary)
            evt.sections.push({
              ...
              type: "text",
              content: h.long_summary,
            });
          if (h.highlights)
            evt.sections.push({
              title: "Event Highlights",
              type: "text",
              content: h.highlights,
            });
          if (h.lessons_learned)
            evt.sections.push({
              title: "Lessons Learned",
              type: "text",
              content: h.lessons_learned,
            });

          // Metadata merge
          evt.attendees_count =
            h.attendees_count ?? evt.attendees_count ?? evt.attendees ?? 0;
          evt.guests = h.guests ?? evt.guests ?? 0;
          evt.budget_spent =
            h.budget_spent ?? evt.budget_spent ?? evt.budget ?? "N/A";
    ...
    <div className="history-wrapper py-4" style={{ background: "#0d0d4d" }}>
      <div className="event-history-page container">
        ...
        <div className="row g-3 mb-4">
          <InfoBox
            title="Attendees"
            value={event.attendees_count ?? event.attendees ?? "0"}
            icon={<FaUsers style={{ color: "#1e3a8a" }} />}
          />
          <InfoBox
            title="Guests"
            value={event.guests ?? "0"}
            icon={<FaIdBadge style={{ color: "#1e3a8a" }} />}
          />
          <InfoBox
            title="Budget Spent"
            value={event.budget_spent ?? event.budget ?? "N/A"}
            icon={<FaMoneyBill style={{ color: "#1e3a8a" }} />}
          />
          <InfoBox
            title="Entry Fee"
            value={event.fees ?? "Free"}
            icon={<FaWallet style={{ color: "#1e3a8a" }} />}
          />
          <InfoBox
            title="Venue"
            value={event.venue ?? "N/A"}
            icon={<FaMapMarkerAlt style={{ color: "#1e3a8a" }} />}
          />
          <InfoBox
            title="Contact"
            value={event.contact ?? "N/A"}
            icon={<FaPhone style={{ color: "#1e3a8a" }} />}
          />
        </div>
