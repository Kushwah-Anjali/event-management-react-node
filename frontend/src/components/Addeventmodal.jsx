...
export default function AddEventModal({ isOpen, onClose, onSubmit }) {
  ...
  const handleChange = e => {
    ...
    switch (name) {
      case "fees":
        ...
        setErrors(prev => ({ ...prev, fees: updatedValue ? "" : "Enter numeric value" }));
        break;
      case "contact":
        updatedValue = value.replace(/\D/g, "").slice(0, 10);
        setErrors(prev => ({
          ...prev,
          contact: updatedValue.length > 0 && updatedValue.length < 10 ? "Must be 10 digits" : ""
        }));
        break;
      case "author":
      case "venue":
        updatedValue = value.replace(/[^a-zA-Z\s]/g, "");
        setErrors(prev => ({ ...prev, [name]: updatedValue ? "" : "Only letters allowed" }));
        break;
      case "date":
        const today = new Date().toISOString().split("T")[0];
        setErrors(prev => ({ ...prev, date: value < today ? "Cannot select past date" : "" }));
        break;
      case "image":
        if (files[0]) {
          const file = files[0];
          const validTypes = ["image/jpeg", "image/png"];
          if (!validTypes.includes(file.type)) setErrors(prev => ({ ...prev, image: "Only JPG/PNG allowed" }));
          else if (file.size > 2 * 1024 * 1024) setErrors(prev => ({ ...prev, image: "Max size 2MB" }));
      ...
      case "required_docs":
        setData(d => {
          ...
          if (checked) {
            if (docs.length >= 4) {
              Swal.fire({ icon: "warning", title: "Limit reached", text: "Max 4 documents allowed", confirmButtonColor: "#0d6efd" });
              return d;
            }
            docs = [...docs, value];
  ...
  const validateStep = s => {
    ...
    if (s === 1) return data.author && data.venue && data.fees && data.contact && !errors.author && !errors.venue && !errors.fees && !errors.contact;
      ...
      <div className="modal-dialog d-flex flex-column" style={{ width: "100%", maxWidth: "550px", maxHeight: "90vh" }}>
        <form className="modal-content shadow rounded-4 d-flex flex-column" style={{ flex: 1, overflow: "hidden" }} onSubmit={handleSubmit}>
          ...
          <div className="modal-footer px-4 pb-4 flex-wrap gap-2">
            ...
            {step === totalSteps - 1 && <button type="submit" className="btn btn-success rounded-pill px-4"><FontAwesomeIcon icon={faPlusCircle} /> Add Event</button>}
          </div>
